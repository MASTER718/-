...
contract Storehouse {
    mapping(address => uint256) public deposit_Eth;
    address token;
    ...
    function deposit(uint256 amount) public {
        uint256 rest = IERC20(token).balanceOf(msg.sender);
        uint256 allowance = IERC20(token).allowance(msg.sender, address(this));
        console.log("----deposit-----", rest, amount, allowance);
        // 判断资产 msg.sender
        // 判断授权
        require(rest >= amount, "insufficient funds");
